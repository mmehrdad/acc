...
{
   public class FactorConfiguration : IEntityTypeConfiguration<Factor>, IDbModelConfiguration
    {
        public void Configure(EntityTypeBuilder<Factor> builder)
        {
            ...
            builder.HasOne(q => q.Person)
                .WithMany(q => q.Factors)
                .HasForeignKey(q => q.PersonId)
                .OnDelete(DeleteBehavior.NoAction);

            builder.Property(m => m.PersonId).ValueGeneratedNever()
                .IsFixedLength(true).HasColumnType("char(18)")
                .HasMaxLength(18);
